                    ...
                    <tbody>
                        @forelse($presensi->details as $detail)
                            <tr>
                                ...
                                <td>
                                    <select name="status_kehadiran" form="pelatih-presensi-detail-{{ $detail->id }}" class="form-select form-select-sm">
                                ...
                                <td>
                                    <input type="text" name="catatan" value="{{ $detail->catatan }}" form="pelatih-presensi-detail-{{ $detail->id }}" class="form-control form-control-sm">
                                </td>
                                <td>
                                    <form id="pelatih-presensi-detail-{{ $detail->id }}" action="{{ route('pelatih.presensi-detail.update', $detail) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted">Belum ada data peserta pada presensi ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
